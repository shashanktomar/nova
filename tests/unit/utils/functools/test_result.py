...
from nova.utils.functools.models import Err, Ok, OkErr, Result, UnwrapError, as_async_result, as_result
...
async def test_and_then_async() -> None:
    ...
    assert (await (await Ok(2).and_then_async(to_err_async)).and_then_async(to_err_async)).err() == 2
    assert (await (await Err(3).and_then_async(sq_async)).and_then_async(sq_async)).err() == 3
...
def test_or() -> None:
    assert Ok(2).or_(Ok(3)).ok() == 2
    assert Ok(2).or_(Err(3)).ok() == 2
    assert Err(2).or_(Ok(3)).ok() == 3
    assert Err(2).or_(Err(3)).err() == 3

    # Chain of ors
    assert Ok(2).or_(Ok(3)).or_(Ok(4)).ok() == 2
    assert Ok(2).or_(Err(3)).or_(Err(4)).ok() == 2
    assert Err(2).or_(Ok(3)).or_(Ok(4)).ok() == 3
    assert Err(2).or_(Err(3)).or_(Ok(4)).ok() == 4
    assert Err(2).or_(Err(3)).or_(Err(4)).err() == 4
